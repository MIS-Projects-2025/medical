...
export const Combobox = ({
    ...
    loadOptions,
    /** Optional: override what's shown in the trigger for the selected option.
     *  Receives the matched option object, returns a string. */
    getDisplayValue,
}) => {
    ...
    const selectedLabel = matchedOption
        ? (getDisplayValue ? getDisplayValue(matchedOption) : matchedOption.label)
        : (value ? String(value) : "");
        ...
        >
            <PopoverTrigger asChild>
                <button
                    ...
                    className={cn(
                        "flex w-full items-center justify-between rounded-md border border-input bg-background px-3 h-9 text-sm",
